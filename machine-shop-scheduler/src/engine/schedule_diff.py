...
class ScheduleDiff:
    ...
    def _operation_diff(self, key):
        ...
        baseline_machine = baseline.get(
            "machine_id"
        )

        replanned_machine = replanned.get(
            "machine_id"
        )

        baseline_operator = baseline.get(
            "operator_id"
        )

        replanned_operator = replanned.get(
            "operator_id"
        )

        baseline_start = baseline.get(
            "start_time"
        )

        replanned_start = replanned.get(
            "start_time"
        )

        baseline_end = baseline.get(
            "end_time"
        )

        replanned_end = replanned.get(
            "end_time"
        )
        ...
        baseline_segments = baseline.get(
            "segments"
        )

        replanned_segments = replanned.get(
            "segments"
        )
        ...
        return {
            ...
            "start_delay_minutes": round(
                start_delay,
                2
            ),
            "completion_delay_minutes": round(
                completion_delay,
                2
            ),
    ...
    def summary(self):
        ...
        for change in changes:
            ...
            total_start_delay += change.get(
                "start_delay_minutes",
                0
            )

            total_completion_delay += change.get(
                "completion_delay_minutes",
                0
            )

        return {
            "baseline_operations": len(
                self.baseline
            ),
            "replanned_operations": len(
                self.replanned
            ),
            "changed_operations": len(
                changes
            ),
            ...
            "total_start_delay_minutes": round(
                total_start_delay,
                2
            ),
            "total_completion_delay_minutes": round(
                total_completion_delay,
                2
            )
        }
