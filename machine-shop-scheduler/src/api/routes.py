...
def load_baseline_data():
    ...
    filepath = os.path.join(base_dir, "data", "generated", "baseline.json")
    
    if not os.path.exists(filepath):
        raise HTTPException(status_code=404, detail="Baseline data not found")
    ...
    return machines, operators, orders, data.get("shop_config", {})

@router.get("/baseline")
def get_baseline_schedule():
    machines, operators, orders, config = load_baseline_data()
    ...
    cost = evaluator.evaluate_total_cost(schedule)
...
@router.post("/replan")
def run_replanner(scenario: Dict[str, Any]):
    machines, operators, orders, config = load_baseline_data()
